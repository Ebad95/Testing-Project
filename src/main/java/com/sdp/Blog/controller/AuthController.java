...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
        if(loginRequest.getUsername() == null || loginRequest.getUsername().length() == 0 || loginRequest.getPassword().length()==0){
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        try {
            ...
            if (signUpRequest.getUsername() == null || signUpRequest.getEmail().length() < 4 || signUpRequest.getPassword().length()<4) {
                return ResponseEntity
                        .badRequest()
                        .body(new MessageResponse("Error: Email should be greater than 4 and password should be greater than 4"));
            }
            else if(signUpRequest.getName() == null || signUpRequest.getEmail().length() >40 || signUpRequest.getUsername().length()<4)
            {
                return ResponseEntity
                        .badRequest()
                        .body(new MessageResponse("Error: Email should not be greater than 40 and username should not be less than 4"));
            }
            else
            {
                user = new User(signUpRequest.getUsername(), signUpRequest.getEmail(), signUpRequest.getName(),
           ...
           // userService.saveUser(user);
            ...
            return ResponseEntity.badRequest().body(new MessageResponse("Error: Unable to add user"));
